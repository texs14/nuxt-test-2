...
const WEBHOOK_URL = 'https://optimizationsonline.org/webhook-test/b9adfbe2-8129-44d8-a437-6e4f9531b76b'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    try { await supabase.storage.createBucket('Videos', { public: false }) } catch {}
    try { await supabase.storage.createBucket('Audios', { public: false }) } catch {}

    // Загружаем исходное видео в Supabase Storage
    const videoBlob = new Blob([videoPart.data], { type: videoPart.type || 'application/octet-stream' })
    ...
    const { error: upVideoErr } = await supabase.storage.from('Videos').upload(videoPath, videoBlob, {
    ...
    const { error: upAudioErr } = await supabase.storage.from('Audios').upload(audioPath, audioBlob, {
    ...
    const { error: upThumbErr } = await supabase.storage.from('Videos').upload(previewPath, thumbBlob, {
    ...
    const audioSigned = await supabase.storage.from('Audios').createSignedUrl(audioPath, 60 * 60 * 24 * 30)
    const videoSigned = await supabase.storage.from('Videos').createSignedUrl(videoPath, 60 * 60 * 24 * 30)
    const previewSigned = await supabase.storage.from('Videos').createSignedUrl(previewPath, 60 * 60 * 24 * 30)
    const audioUrl = audioSigned.data?.signedUrl || supabase.storage.from('Audios').getPublicUrl(audioPath).data.publicUrl
    const videoUrl = videoSigned.data?.signedUrl || supabase.storage.from('Videos').getPublicUrl(videoPath).data.publicUrl
    const previewUrl = previewSigned.data?.signedUrl || supabase.storage.from('Videos').getPublicUrl(previewPath).data.publicUrl
