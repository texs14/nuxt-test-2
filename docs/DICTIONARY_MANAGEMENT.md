...
Система управления словарем позволяет пользователям добавлять новые слова и редактировать существующие записи в таблице `dictionary` базы данных Supabase. Функционал интегрирован в компонент интерактивных слов, который отображает переводы при клике на тайское слово.
...
|------|-----|--------------|----------|
| `id` | bigint | Да | Первичный ключ (автоинкремент) |
| `word_th` | text | Да | Тайское слово |
| `translation` | text[] | Да | Массив переводов |
| `transcription_en` | text | Нет | Транскрипция на английском |
| `synonyms` | text[] | Нет | Массив синонимов |
| `antonyms` | text[] | Нет | Массив антонимов |
| `links` | text[] | Нет | Массив ссылок на внешние ресурсы |
| `examples` | jsonb | Нет | JSON-массив примеров использования |
| `created_at` | timestamptz | Нет | Дата создания записи |
