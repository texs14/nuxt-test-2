...
Система автоматической генерации словарных записей для тайских слов с использованием GPT-4 и синтеза аудио через Resemble AI.

## Возможности

- ✅ Автоматическая генерация полных словарных записей через GPT-4
...
**Composables:**
- `app/composables/useOpenAI.ts` - базовый composable для работы с OpenAI API
- `app/composables/useDictionaryAI.ts` - генерация словарных записей через GPT-4

**Server API Endpoints:**
- `server/api/openai/chat.post.ts` - прокси для OpenAI Chat Completions API
...
1. **Generating (🤖)** - Генерация перевода и примеров через GPT-4
...
1. **Rate Limits** - OpenAI API имеет ограничения на количество запросов
2. **Стоимость** - Каждая генерация использует токены GPT-4 (примерно $0.03-0.06 за слово)
3. **Точность** - GPT-4 может ошибаться, рекомендуется проверка сгенерированных данных
...
- [ ] Поддержка других моделей (GPT-4 Turbo, Claude)
